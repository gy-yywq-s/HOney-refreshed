  ...
  return (
    <IonPage data-scroll-model="FRAMED_SCROLL">
      ...
      <IonContent className="screen-content" id="main-view">
        <main className="screen-inner" data-scroll-owner="notes-and-posts">
          {section === "private" ? <PrivateNotes notes={notes} onRemove={(id) => setConfirm({ kind: "note", id })} /> : loading ? <LoadingState /> : error ? <div className="notice error-notice">{error}</div> : <SharedPosts posts={shared} onRevoke={(id) => setConfirm({ kind: "post", id })} />}
        </main>
      </IonContent>
...
function PrivateNotes({ notes, onRemove }: { notes: PrivateNote[]; onRemove(id: string): void }) {
  ...
  return <IonList lines="full">{notes.map((note) => <IonItem key={note.id}><IonLabel className="ion-text-wrap"><p className="eyebrow">{note.state === "cooldown" ? "Kept for later" : note.targetLabel}</p><h2>{note.body}</h2><p>{new Date(note.savedAt).toLocaleString("en-GB")}</p></IonLabel><IonButton slot="end" fill="clear" color="danger" onClick={() => onRemove(note.id)}>Remove</IonButton></IonItem>)}</IonList>;
}
