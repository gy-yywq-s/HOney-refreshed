...
HOney runs as **four Node services behind one small edge proxy**, all **directly under the linux
...
| Access (Web Access: gates, permits; portal egress only) | systemd `honey-access.service` → `127.0.0.1:8874`; `/home/honey/data/access.db`; `IPAddressAllow=` pins egress to the portal — see [`../architecture/web-access.md`](../architecture/web-access.md) |
| Access secrets env | `/home/honey/.secrets/access.env` (0600: HONEY_INTERNAL_SECRET, HONEY_ACCESS_DB_PATH, HONEY_KEYS_DIR, PORT=8874, NODE_ENV) — never HONEY_SECRET, never a school credential |
...
| Service keys | `/home/honey/data/keys/` (0700): `issuer.jwk.json` (blind-eligibility issuer, 0600, never in git; `pnpm --filter @honey/backend issuer:keygen` with the service env loaded), `core-signing.private.json` (Core's Ed25519 capability signer, created on first start), `access-sealing.private.json` (the Access Service's HPKE key, created on its first start) + the public descriptors written for peer processes |
...
sudo systemctl restart honey honey-community honey-access honey-edge && sleep 3 \
  && curl -s https://honey.gaelisus.com/api/health && curl -s https://honey.gaelisus.com/community/health \
  && curl -s https://honey.gaelisus.com/access/health
```
...
**Web Access is OFF by default** on a fresh `access.db`: turn it on in Dash › Web Access only for
the controlled real verification (spec §26.3), then pause it again. Start order after a key wipe:
`honey-access` first (it writes its sealing public key), then `honey` (it reads it when issuing a
capability; until then `/api/access/session` answers `access_unavailable`).
