...
Service version: feat-web-access. Driven through the real HTTP surface of the Access Service against the in-process mock portal (spec §26.2). Each injection uses a fresh journal and a fresh mock portal; the door-request column is the number of physical requests the mock actually received.
...
| 6 | school rejects: portal answers status:1 'no permission' | stream ends rejected; journal REJECTED/portal_rejected; 1 door request | 200 rejected | REJECTED / portal_rejected | 1 | The school declined this request. Nothing was opened. | ✅ |
